...
//layout (binding = 1) uniform sampler2D AlphaMapSampler;
...
void main()
{
	float sampledAlpha = texture(AlphaMapSampler, IN.TextureCoordinate).a;

	if (IN.FogAmount == 1.0f)
	{
		Color = vec4(FogColor.rgb, sampledAlpha); 
		return;
	}
