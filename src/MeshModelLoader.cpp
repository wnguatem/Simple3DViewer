...
namespace Rendering
{
	...
	MeshModelLoader::MeshModelLoader(Game& game, Camera& camera)
...
//        }
	...
	void MeshModelLoader::Initialize()
    {
...
////        mVertexBuffer.push_back(tmpvertexBuffer);
		...
		mKeyboardHandler = std::bind(&MeshModelLoader::OnKey, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
...
////        mAmbientLight = new Light(*mGame);
	...
	void MeshModelLoader::Update(const GameTime& gameTime)
    {
	...
	void MeshModelLoader::UpdateAmbientLight(const GameTime& gameTime)
    {
...
//        }
	...
	void MeshModelLoader::Draw(const GameTime& gameTime)
    {
		...
		//glDrawArrays(GL_POINTS, 0, mIndexCount);
		glDrawElements(mActive_mode, mIndexCount, GL_UNSIGNED_INT, 0);
...
//        }
	...
	void MeshModelLoader::CreateVertexBuffer(const std::vector<VertexPositionTexture> &vertices, const std::vector<unsigned int> &nr_ver_per_img, std::vector<GLuint>& vertexBuffer)
    {
	...
	void MeshModelLoader::CreateIndexBuffer(const std::vector<unsigned int> &indices, const std::vector<unsigned int> &nr_ver_per_img, std::vector<GLuint>& indexBuffer)
    {
        //int ver_count_indx = 0;
        //mIndexCount.resize(m_img_nr.size());
        //for (size_t i = 0; i < m_img_nr.size(); i++)
	...
	void MeshModelLoader::CreateVertexBuffer(const Mesh& mesh, GLuint& tmpvertexBuffer) const
    {
	...
	void MeshModelLoader::CreateVertexBuffer(VertexPositionTexture* vertices, GLuint vertexCount, GLuint& tmpvertexBuffer) const
    {
        glGenBuffers(1, &tmpvertexBuffer);
        glBindBuffer(GL_ARRAY_BUFFER, tmpvertexBuffer);
	...
	unsigned int MeshModelLoader::VertexSize() const
    {
	...
	void MeshModelLoader::OnKey(int key, int scancode, int action, int mods)
    {
