...
namespace Rendering
{
	...
	FacadeFileModeler::FacadeFileModeler(Game& game, Camera& camera)
		...
		mMainFacadeIndexBuffer(0), mContentFolder(game.getGameContentFolder())
	{
	}
...
//				}
	...
	void FacadeFileModeler::initMainFacadeQuad()
	{
		...
		{
			VertexPositionColor(vec4(-1.0f, 0.1f, 0.0f, 1.0f), vec4(0.0f, 1.0f, 0.0f, 1.0f)),
			VertexPositionColor(vec4(-1.0f, 2.1f, 0.0f, 1.0f), vec4(0.0f, 1.0f, 0.0f, 1.0f)),
			VertexPositionColor(vec4(1.0f, 2.1f, 0.0f, 1.0f), vec4(0.0f, 1.0f, 0.0f, 1.0f)),
			VertexPositionColor(vec4(1.0f, 0.1f, 0.0f, 1.0f), vec4(0.0f, 1.0f, 0.0f, 1.0f))
		};
		...
		CreateVertexBuffer(vertices, sizeof(vertices) / sizeof(VertexPositionColor), mMFQuadVertexBuffer);
		...
		mMFQuadIndexCount = sizeof(indices) / sizeof(unsigned int);

		glGenBuffers(1, &mMFQuadIndexBuffer);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mMFQuadIndexBuffer);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int)* mMFQuadIndexCount, indices, GL_STATIC_DRAW);

		glGenVertexArrays(1, &mMFQuadVertexArrayObject);
		glBindVertexArray(mMFQuadVertexArrayObject);
	...
	void FacadeFileModeler::Draw(const GameTime& gameTime)
	{
		////draw the main Quad
		//glBindVertexArray(mMFQuadVertexArrayObject);
		//glBindBuffer(GL_ARRAY_BUFFER, mMFQuadVertexBuffer);
		//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mMFQuadIndexBuffer);
		...
		//glFrontFace(GL_CCW);
		//glDrawArrays(GL_LINE_STRIP, 0, mMFQuadIndexCount);
	...
	void FacadeFileModeler::CreateVertexBuffer(VertexPositionColor* vertices, GLuint vertexCount, GLuint& vertexBuffer)
	{
		glGenBuffers(1, &vertexBuffer);
		glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer);
		glBufferData(GL_ARRAY_BUFFER, sizeof(VertexPositionColor) * vertexCount, &vertices[0], GL_STATIC_DRAW);
	}
