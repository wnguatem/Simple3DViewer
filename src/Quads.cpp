...
namespace Rendering
{
    //RTTI_DEFINITIONS(Quads)
    
    Quads::Quads(Game& game, Camera& camera)
    ...
    void Quads::Draw(const GameTime& gameTime)
    {
        glBindVertexArray(mVertexArrayObject);
    ...
    void Quads::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer)
    {
    ...
    void Quads::CreateVertexBuffer(const pcl::PointCloud<pcl::PointXYZRGBNormal>& cloud, GLuint& vertexBuffer)
    {
        //const std::vector<vec3>& sourceVertices = mesh.Vertices();
